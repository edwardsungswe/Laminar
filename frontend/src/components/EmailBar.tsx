...
export default function EmailBar() {
    const { inbox, setInbox, emailInView } = useEmailDisplayContext()

    return (
        <div className={`flex flex-col gap-4 w-${emailInView ? "1/4" : "full"} h-full border-x-1 border-med`}>
            <div className="flex gap-4 items-center w-full h-1/16 min-h-18 px-4 border-b-1 border-med">
                <h1 className="text-2xl">Inbox</h1>
                <SwitchButton state={inbox} setState={setInbox} buttonNames={["All Mail", "Unread"]} stateNames={["all", "unread"]} />
            </div>
